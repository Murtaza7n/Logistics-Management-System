...
@section('page-title', 'SPO/Cargo Officers Management')
...
<div class="card">
    <div class="card-header">
        <form method="GET" class="row g-2">
            ...
            <div class="col-md-2">
                <a href="{{ route('master-data.cargo-officers') }}" class="btn btn-secondary w-100">Reset</a>
            </div>
        </form>
    </div>
    <div class="card-body">
        ...
        <div class="table-responsive">
            <table class="table table-striped table-hover">
                ...
                <tbody>
                    @forelse($cargoOfficers as $officer)
                    <tr>
                        <td><strong>{{ $officer->officer_code }}</strong></td>
                        ...
                        <td>
                            <button type="button" class="btn btn-sm btn-info" onclick="editCargoOfficer({{ $officer->id }})">Edit</button>
                            <form action="{{ route('master-data.cargo-officers.delete', $officer->id) }}" method="POST" class="d-inline" onsubmit="return confirm('Are you sure?')">
                                @csrf
                                @method('DELETE')
...
<script>
let cargoOfficersData = @json($cargoOfficers->items());
...
function editCargoOfficer(id) {
    ...
    document.getElementById('cargoOfficerForm').action = '{{ route("master-data.cargo-officers.update", ":id") }}'.replace(':id', id);
...
document.getElementById('addCargoOfficerModal').addEventListener('hidden.bs.modal', function() {
    ...
    document.getElementById('cargoOfficerForm').action = '{{ route("master-data.cargo-officers.store") }}';
